...
var EmojiParty = (() => {
    ...
    function Start() {
        ...
        const { FileUploader, ParserModule, PermissionEvaluator, ChannelStore, UserStore, MessageCache, PendingReplyDispatcher, original_useEmojiSelectHandler, MessageAttachmentManager } = Discord;
        ...
        async function renderUploadEmojiParty(channelId, message, messageParts, mentions, callOriginal) {
            console.log("In renderUploadEmojiParty function");
            ...
            for (const [emojiId, emojiPart] of [...uniqueEmojiIds, ...uniqueEmoteAssets]) {
                ...
                if(cachedImage) {
                    ...
                    if(emojiPart.type === 'customEmoji') {
                        ...
                        decodePromise = await imageElement.decode();
                    }
                    decodePromises.push({ decodePromise, emojiId, imageElement });
                }
            }

            Promise.all(decodePromises.map(x => x.decodePromise));
            ...
            canvas.toBlob(blob => {
                ...
                MessageAttachmentManager.addFiles({
                    files: [{
                        file: new File([blob], "sticker.png"),
                        platform: 1,
                        isThumbnail: false
                    }],
                    channelId: channelId,
        ...
        sendMessageHook = function(channelId, message, waitForReadyState, mentions) {
            console.log("In sendMessageHook");
            ...
            console.log("User has necessary permissions: ", tryToProcess);
            ...
            if(tryToProcess) {
                ...
                if(emojiCount >= TriggerEmojiCount || message.invalidEmojis.length !== 0 || message.validNonShortcutEmojis.some(x => !x.available)) {
                    console.log("Rendering emoji party");
